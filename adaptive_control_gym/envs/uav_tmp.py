...
class HoverEnv(gym.Env):
    def __init__(self, 
        ...
        rotate_mass_mean:float = 0.1, rotate_mass_std:float=0.0,
        ...
        self.rotate_mass_mean, self.rotate_mass_std = rotate_mass_mean, rotate_mass_std
        self.rotate_mass_min, self.rotate_mass_max = 0.02, 2.0
        ...
        self.res_dyn_mlp = ResDynMLP(input_dim=dim*3+res_dyn_param_dim+2, output_dim=dim+1).to(self.device)
        ...
        self.state_dim = 4*dim+2*dim*self.obs_traj_len + 2
        if expert_mode:
            self.state_dim += (res_dyn_param_dim+dim*2+2)
        self.action_dim = dim
    ...
    def _get_initial_state(self, size = None):
        ...
        if self.ood_mode:
            ...
            rotate_mass = torch.abs(torch.randn((size, self.dim), device=self.device)) * self.rotate_mass_std + self.rotate_mass_mean
            delay = torch.abs(torch.randn((size, 1), device=self.device)) * self.delay_std + self.delay_mean
            decay = torch.abs(torch.randn((size, self.dim+1), device=self.device)) * self.decay_std + self.decay_mean
            ...
            theta = torch.rand((size, self.dim), device=self.device) * np.pi - np.pi
        else:
            ...
            rotate_mass = torch.randn((size, self.dim), device=self.device) * self.rotate_mass_std + self.rotate_mass_mean
            delay = torch.randn((size, 1), device=self.device) * self.delay_std + self.delay_mean
            decay = torch.randn((size, self.dim+1), device=self.device) * self.decay_std + self.decay_mean
            res_dyn_param = torch.randn((size, self.res_dyn_param_dim), device=self.device) * self.res_dyn_param_std + self.res_dyn_param_mean
            theta = torch.rand((size, self.dim), device=self.device) * 2 * np.pi - np.pi
        ...
        w = torch.zeros((size, 1), device=self.device)
        return x, theta, v, mass, rotate_mass, delay, decay, res_dyn_param, w
    ...
    def step(self, action):
        ...
        actual_action = force*self.model_delay_alpha + self.force*(1-self.model_delay_alpha)
        self.force = torch.cat([actual_action[:,[0]]*torch.cos(self.theta), actual_action[:,[0]]*torch.sin(self.theta)], dim=-1)
        self.moment = actual_action[:,[1]]
        
        self.force += self.disturb[:, :2]
        self.moment += self.disturb[:, 2:]
        self.decay_force = self.decay[:, :2] * self.v
        self.force -= self.decay_force
        self.decay_moment = self.decay[:, 2:] * self.w
        self.moment -= self.decay_moment
        self.res_dyn_force = self.res_dyn_mlp(torch.cat([self.x, self.v*0.3, self.theta, self.w*0.3, action, self.res_dyn_param], dim=-1)) * self.res_dyn_scale
        self.force += self.res_dyn_force[:, :2]
        self.moment += self.res_dyn_force[:, 2:]
        
        self.x += self.v * self.tau
        self.theta += self.w * self.tau
        self.v += self.force / self.mass * self.tau
        self.w += self.moment / self.rotate_mass * self.tau
        self.x = torch.clip(self.x, self.x_min, self.x_max)
        self.w = (self.w + np.pi) % (2 * np.pi) - np.pi
        reward = 1.0 - torch.norm(self.x-self.traj_x[...,self.step_cnt],dim=1) - torch.norm(self.v-self.traj_v[...,self.step_cnt],dim=1)*0.2
        reward -= (torch.norm(self.theta,dim=1)*0.5 + torch.norm(self.w,dim=1)*0.05)
    ...
    def reset(self):
        self.step_cnt = 0
        self._set_disturb()
        self.x, self.theta, self.v, self.mass, self.rotate_mass, self.delay, self.decay, self.res_dyn_param, self.w = self._get_initial_state()
        ...
        self.moment = torch.zeros((self.env_num, 1), device=self.device)
    ...
    def _get_obs(self):
        ...
        obs = torch.concat([self.x, self.v*0.3, err_x, err_v*0.3, self.theta, self.w, future_traj_x.reshape(self.env_num,-1), future_traj_v.reshape(self.env_num, -1)], dim=-1)
        if self.expert_mode:
            obs = torch.concat([obs, self.mass, self.rotate_mass, self.delay/self.delay_max, self.decay/self.delay_max, self.res_dyn_param], dim=-1)
        return obs

    def _set_disturb(self):
        self.disturb = torch.randn((self.env_num,self.dim+1), device=self.device) * self.disturb_std + self.disturb_mean
...
def test_hover(env, policy, save_path = None):
    state = env.reset()
    x_list, v_list, a_list, force_list, disturb_list = [], [], [], []
    decay_list, res_dyn_list, mass_list, delay_list = [], [], [], []
    res_dyn_param_list, traj_x_list, traj_v_list, r_list = [], [], [], []
    theta_list, w_list, rotate_mass_list, moment_list = [], [], [], []
    done_list = []
    ...
    for t in range(time_limit):
        ...
        theta_list.append(state[0,env.dim*2].item())
        w_list.append(state[0,env.dim*2+1].item())
        rotate_mass_list.append(env.rotate_mass[0,0].item()*10)
        moment_list.append(env.moment[0,0].item())
    ...
    axs[env.dim].set_title(f"angle parameters")
    axs[env.dim].plot(theta_list, label="theta")
    axs[env.dim].plot(w_list, label="w", alpha=0.3)
    axs[env.dim].plot(rotate_mass_list, label="rotate_mass*10", alpha=0.3)
    axs[env.dim].plot(moment_list, label="moment", alpha=0.3)
    for i in range(env.dim):
        fig_id = env.dim + i + 1
        axs[fig_id].set_title(f"force measurement dim={i}")
        axs[fig_id].plot(res_dyn_numpy[:,i], label="res_dyn")
        axs[fig_id].plot(a_numpy[:,i], label="action", linestyle='--', alpha=0.5)
        axs[fig_id].plot(force_array[:,i], label="force", alpha=0.5)
        axs[fig_id].plot(disturb_array[:,i], label="disturb", alpha=0.2)
        axs[fig_id].plot(decay_array[:,i], label="decay", alpha=0.3)
