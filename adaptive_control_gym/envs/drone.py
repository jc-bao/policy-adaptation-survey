...
class DroneEnv(gym.Env):
    def __init__(self, 
        dim: int = 3, env_num: int = 1, gpu_id: int = 0, seed:int = 0, 
    ...
    def _get_initial_state(self, size = None):
        ...
        if self.ood_mode:
            ...
            x[:, 2] = torch.rand((size), device=self.device) * np.pi
    ...
    def step(self, action):
        ...
        theta = self.x[:,[2]]
        F, M = force[:,[0]], force[:,[1]]
        force = torch.cat([F*torch.cos(theta), F*torch.sin(theta), M], dim=-1)
...
def test_drone(env, policy, save_path = None):
    state = env.reset()
    ...
    if save_path == None:
        ...
        save_path = f"{package_path}/../results/drone.png"
